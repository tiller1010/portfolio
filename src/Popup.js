...
class Popup extends Component {
	...
	handleSubmit(event){
		...
		// Production: https://email-capture.tech
		xhr.open('GET', `https://email-capture.tech/api/email-submission/store?email=${encodeURIComponent(this.state.email)}`); // Will need to update this url
	...
	handleMessageSubmit(event){
		...
		// Local: http://email-capture.local
		// Production: https://email-capture.tech
		xhr.open('GET', `https://email-capture.tech/api/email-message/store?email_address=${encodeURIComponent(sessionStorage.getItem('email'))}&message=${encodeURIComponent(this.state.message)}`); // Will need to update this url
		xhr.onreadystatechange = function(){
			if(this.readyState === XMLHttpRequest.DONE && this.status === 200){
